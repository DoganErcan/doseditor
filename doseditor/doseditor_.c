 ...
 * --------------------------------------------------------------- */
static void ab_puts( Abuf* a, const char* s ) {
	ab_append( a, s, strlen( s ) );
}
 ...
 * 53. ed_prompt()
 * --------------------------------------------------------------- */
static int ed_prompt_ex( const char* prompt, char* buf, size_t buflen, int keep, int allow_empty ) {
	if ( !buflen ) return 0;
	if ( !keep ) buf[ 0 ] = '\0';
	buf[ buflen - 1 ] = '\0';
	...
	for ( ;; ) {
		...
		size_t width = ( size_t ) E.screen_cols - label;
		if ( width < 1 ) width = 1;
		size_t off = pos >= width ? pos - width + 1 : 0;
		char seq[ 1024 ]; DWORD n;
		snprintf( seq, sizeof seq, "\x1b[%d;1H\x1b[m%.*s%.*s\x1b[K\x1b[%d;%zuH",
			E.screen_rows + 3, ( int ) label, prompt, ( int ) width, buf + off,
...
static int ed_prompt( const char* prompt, char* buf, size_t buflen ) {
	return ed_prompt_ex( prompt, buf, buflen, 0, 0 );
}

static int ed_save_as( void ) {
	char name[ MAX_PATH ];
	strcpy( name, E.filename );
	if ( !ed_prompt_ex( "Speichern unter (Esc=Abbruch): ", name, sizeof name, 1, 0 ) ) return 0;
	if ( _stricmp( name, E.filename ) && GetFileAttributesA( name ) != INVALID_FILE_ATTRIBUTES ) {
		char answer[ 8 ];
		if ( !ed_prompt( "Datei ersetzen? (j/n): ", answer, sizeof answer ) ||
...
static int ed_confirm_discard( void ) {
	if ( !E.dirty ) return 1;
	char answer[ 8 ];
	if ( !ed_prompt( "Aenderungen: (s)peichern / (v)erwerfen / Esc: ", answer, sizeof answer ) ) return 0;
...
static void ed_search( int find_next ) {
	if ( !find_next ) {
		char query[ 64 ];
		if ( !ed_prompt( "Suche nach: ", query, sizeof( query ) ) ) {
			return;
		}
...
static void ed_search_replace( void ) {
	...
	if ( !ed_prompt( "Suchen: ", query, sizeof( query ) ) ) return;
	if ( !ed_prompt_ex( "Ersetzen durch (auch leer, Esc=Abbruch): ", replacement, sizeof replacement, 0, 1 ) ) return;
...
static void ed_goto_line( void ) {
	char numbuf[ 32 ];
	if ( ed_prompt( "Gehe zu Zeile: ", numbuf, sizeof( numbuf ) ) ) {
		char* end;
		errno = 0;
