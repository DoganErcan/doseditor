...
enum { SYN_COMMENT = 1, SYN_STRING = 2, SYN_CHAR = 4,
...
static int asm_zahl(const char* s,size_t len)
{
 ...
 if(!len)return 0;
 if(s[0]=='$'||s[0]=='#'){base=16;a=1;}
 else if(s[0]=='%'){base=2;a=1;}
 else if(len>2 && s[0]=='0' && (s[1]=='x'||s[1]=='X')){base=16;a=2;}
 else if(len>2 && s[0]=='0' && (s[1]=='b'||s[1]=='B')){base=2;a=2;}
 else if(isdigit((unsigned char)s[0])) {
  int suffix=toupper((unsigned char)s[len-1]);
  if(suffix=='H'){base=16;b--;}
  else if(suffix=='B'){base=2;b--;}
  else if(suffix=='O'||suffix=='Q'){base=8;b--;}
  else if(suffix=='D')b--;
 } else return 0;
 if(a==b)return 0;
...
static void asm_zeile_faerben(uint8_t* hl,Line* l,int previous,int* out)
{
 ...
 while(i<n) {
  ...
  if(asm_symbolzeichen(c)||((c=='#'||c=='%')&&i+1<n&&isxdigit((unsigned char)s[i+1]))) {
   ...
   } else if(first&&asm_wort_in_liste(word,wlen,z80_direktiven)){kind=HL_PREPROC;first=0;}
   else if(asm_zahl(s+start,len))kind=HL_NUMBER;
  ...
  if(c==':'){first=1;syntax_markieren(hl,i,i+1,HL_LABEL);}
	...
	while ( i < n )
	{
		...
		if ( isdigit ( c ) || ( c == '.' && i + 1 < n && isdigit ( ( unsigned char ) s [ i + 1 ] ) ) )
		{
			...
			while ( i < n )
			{
				...
				if ( ( d == '+' || d == '-' ) &&
					 ( hex ? ( last == 'p' || last == 'P' ) : ( last == 'e' || last == 'E' ) ) ) { i++; continue; }
				break;
			}
		...
		if ( isalpha ( c ) || c == '_' )
		{
			...
			} else if ( macro_name ) { kind = HL_CONSTANT; macro_name = 0; }
			else if ( wort_in_liste ( s + begin, len, c_keywords ) ) kind = HL_KEYWORD;
