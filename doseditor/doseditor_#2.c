 ...
 * 25. ist_trennzeichen()
 * --------------------------------------------------------------- */
static int ist_trennzeichen ( int c )
{
...
static void zeile_syntaxhervorhebung_berechnen ( uint8_t* hl, Line* l, int prev_open, int* out_open )
{
	...
	for ( size_t i = 0; i < l->len; i++ )
	{
		...
		if ( prev_sep && ( isalpha ( ( unsigned char ) c ) || c == '_' ) )
		{
			...
			if ( E.syntax == 1 )
			{
				if ( wort_in_liste ( l->chars + start, wlen, c_keywords ) ) hltype = HL_KEYWORD;
				else if ( wort_in_liste ( l->chars + start, wlen, c_types ) ) hltype = HL_TYPE;
			}
...
/* ---------------------------------------------------------------
 * 29. syntaxzustand_aktualisieren()
 * --------------------------------------------------------------- */
